  ...
  methods: {
    ...
    async deletePayment(id, paymentId) {
      if (window.confirm("אשר מחיקת תשלום")) {
        ...
        let delInvoiceId = await apiService.clientGetEntities(INVOICE_MODEL, {filter:{supplierId: this.supplier.table_code, paymentId: paymentId}})
    ...
    async retrieveInvoices() {
			this.loading = true
      let response = await apiService.clientGetEntities(INVOICE_MODEL, {filter:{supplierId: this.payment.supplierId}})
      if (response.data.length > 0) {
